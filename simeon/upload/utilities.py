...
from simeon.exceptions import BigQueryNameException, MissingSchemaException
...
def local_to_bq_table(fname: str, file_type: str, project: str) -> str:
    ...
    return "{p}.{d}{s}.{t}".format(d=dataset, s=suffix, p=project, t=table)
...
def make_bq_load_config(
    ...
    :param max_bad_rows: The number of bad rows to tolerate when loading the data
    ...
    config = bigquery.LoadJobConfig(
        ...
        source_format=format_,
        create_disposition=create_disp,
        write_disposition=create_disp,
        field_delimiter=delim,
        skip_leading_rows=skips,
        max_bad_records=max_bad_rows,
        ignore_unknown_values=True,
