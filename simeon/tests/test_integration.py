...
class SimeonCLIIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log_file = os.path.join(self.this_dir, "fixtures", "file.log")
        self.config_file = os.path.join(self.this_dir, "fixture", "config.ini")
        self.good_subcommands = ["download", "list", "push", "split"]
...
class SimeonGeoIPCLIIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.this_dir = os.path.dirname(os.path.abspath(__file__))
        self.log_file = os.path.join(self.this_dir, "fixtures", "file.log")
        self.config_file = os.path.join(self.this_dir, "fixtures", "config.ini")
        self.un_file = os.path.join(self.this_dir, "fixtures", "un_data.csv")
        self.output_file = os.path.join(self.this_dir, "fixtures", "geoip.json.gz")
        self.global_options = [
            "--log-file {f}".format(f=self.log_file),
            "--config-file {f}".format(f=self.config_file),
            "--quiet",
    ...
    def test_geoip_global_options(self):
        ...
        for option in self.global_options:
            ...
            msg = "Checking that option {o} for simeon-geoip exists".format(o=option)
            with self.subTest(msg):
                self.proc = _run_command(cmd)
                self.assertEqual(self.proc.returncode, 0)
                self.tearDown()
    ...
    def test_geoip_extract_options(self):
        ...
        for option in self.global_options:
            ...
            msg = "Checking that option {o} for extract exists".format(o=option)
            with self.subTest(msg):
                self.proc = _run_command(cmd)
                self.assertEqual(self.proc.returncode, 0)
                self.tearDown()
    ...
    def test_geoip_merge_options(self):
        ...
        for option in self.global_options:
            ...
            msg = "Checking that option {o} for merge exists".format(o=option)
            with self.subTest(msg):
                self.proc = _run_command(cmd)
                self.assertEqual(self.proc.returncode, 0)
                self.tearDown()
