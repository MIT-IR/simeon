...
class TestDownloadUtilities(unittest.TestCase):
    ...
    def setUp(self):
        self.dead_letter_text = os.path.join("dead_letters", "dead_letter_queue")
        ...
        self.good_module_id_recs = [
            ...
            {
                "event": {"id": "i4x://ORGx/Course1x/foo/bar", "new": "baz"},
                "event_type": "seq_goto",
                "event_source": "browser",
            },
            {
            ...
            {
                "event": {"id": "input_i4x-ORGx-Course1x-problem-foo_0_0="},
                "event_type": "",
                "event_source": "browser",
            },
            {
        ...
        self.bad_module_id_recs = [
            {"event": dict(), "event_type": "", "event_source": ""},
            {"event": "", "event_type": "add_resource"},
            {"event": "", "event_type": "delete_resource"},
            {"event": "", "event_type": "recommender_upvote"},
            {"event": {"id": None}, "event_type": ""},
            {"event": "", "event_type": "", "event_source": ""},
        ]
        ...
        self.sql_file_names = [
            ...
            {"input": "prod-foo.mongo.gpg", "good-output": "foo/prod/forum.mongo.gpg"},
        ]
        self.course_id_records = [
            {"course_id": "MITx+CourseX+1T2000"},
            {"context": {"course_id": "MITx+CourseX+1T2000"}},
            {"event_source": "browser", "page": "https://edx.org/MITx+CourseX+1T2020"},
    ...
    def test_good_file_dates(self):
        ...
        self.assertListEqual(fdates, ["2021-01-01", "2021-01-02", "2021-01-03"])
    ...
    def test_bad_decryption(self):
        ...
        with warnings.catch_warnings():
            ...
            with self.assertRaises(DecryptionError):
                down_utils.decrypt_files("thereisnowaythisfileexists.gpg", False)

    def test_good_module_id_recs(self):
        """
        Test that down_utils.get_module_id works with valid records
        """
        for record in self.good_module_id_recs:
            msg = "Getting module ID of {r}".format(r=record)
            with self.subTest(msg):
                self.assertIsNotNone(down_utils.get_module_id(record))

    def test_bad_module_id_records(self):
        """
        Test that down_utils.get_module_id returns None with valid records
        """
        for record in self.bad_module_id_recs:
            msg = "Getting module ID of {r}".format(r=record)
            with self.subTest(msg):
                self.assertIsNone(down_utils.get_module_id(record))
    ...
    def test_bad_json_log_lines(self):
        ...
        for i, line in enumerate(self.bad_json_log_lines, 1):
            ...
            with self.subTest(msg.format(r=line)):
                ...
                self.assertIn(self.dead_letter_text, out.get("filename", ""))
    ...
    def test_good_log_lines(self):
        ...
        for i, line in enumerate(self.good_log_lines, 1):
            ...
            with self.subTest(msg.format(r=line)):
                out = logs.process_line(line, i)
                self.assertNotIn(self.dead_letter_text, out.get("filename", ""))
    ...
    def test_good_sql_course_ids(self):
        ...
        for course_str in self.good_sql_course_ids:
            ...
            with self.subTest(msg):
                self.assertIsNotNone(down_utils.get_sql_course_id(course_str))
    ...
    def test_unexpected_extension(self):
        ...
        with self.assertRaises(ValueError):
            down_utils.format_sql_filename(self.sql_file_unexpected_ext)
    ...
    def test_sql_filenames(self):
        ...
        for input_dict in self.sql_file_names:
            msg = "Testing format_sql_filename with {f}".format(f=input_dict["input"])
            with self.subTest(msg):
                self.assertEqual(
                    down_utils.format_sql_filename(input_dict["input"]),
    ...
    def test_course_ids(self):
        ...
        for record in self.course_id_records:
            ...
            with self.subTest(msg):
                self.assertIsNotNone(down_utils.get_course_id(record))
